...
export function NotificationList({notifications, closeForm}) {
    ...
    const onCLickDeleteNotification = useCallback((id) => {
        dispatch(requestDeleteNotificationActionCreator(id))
    }, [dispatch])

    return (
        <div class="notification_list__form">
            <img src={close_icon} className='notification_list__form__close_icon' alt='Удалить' onClick={closeForm}/>
            ...
            {notifications.map((notification) => (
                <div class="notificaton_list__item">
                    ...
                    <img src={close_icon} className='item__delete_icon' alt='Удалить' onClick={() => onCLickDeleteNotification(notification.id)}/>
                </div>    
            ))}
        </div>
    )
